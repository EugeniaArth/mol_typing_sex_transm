...
# Load the summary table (CSV file)
summary_table_path = '/Users/eugenianikonorova/Genomes_mol_typing/Chlamydiales/snp/L2_434_Bu_final_snp_table.csv'
...
print("Summary Table Columns:", summary_df.columns)
print("Gene Info Table Columns:", gene_info_df.columns)

# Perform the merge between the summary table and the gene info table
merged_df = pd.merge(summary_df, gene_info_df[['old_locus_tag', 'protein_id']], how='left', left_on='Locus tag', right_on='old_locus_tag')
...
if 'protein_id' in merged_df.columns:
    # Replace the "Protein ID" in the summary table with the "protein_id" from the gene info table
...
output_path = '/Users/eugenianikonorova/Genomes_mol_typing/Chlamydiales/snp/L2_434_Bu_final_snp_table.csv'
