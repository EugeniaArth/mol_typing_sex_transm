...
# не использовала этот вариант в итоге - делаем без рекалибровки!!!
...
for f in ~/Genomes_mol_typing/Chlamydiales/GATK4/calling/*.sorted.bam # for each sample
  do
    i=${f%%.sorted.bam}
...
# Base Quality Score Recalibration (BQSR) #используем последний bam файл, который первоначально пошел на vcf
gatk --java-options "-Xmx4G" BaseRecalibrator \
        -R Reference.fna \
        -I ${i}.marked_dup.bam \
        --known-sites ${i}.snp.passed.vcf \
        --known-sites ${i}.indels.passed.vcf \
        -O recal_data.table
echo BQSR is performed for ${i}

#Apply BQSR This step applies the recalibration computed in the first BQSR step to the bam file.
#This recalibrated bam file is now analysis-ready.
gatk --java-options "-Xmx4G" ApplyBQSR \
        -R Reference.fna  \
        -I ${i}.marked_dup.bam \
        -bqsr recal_data.table \
        -O ${i}.recal_reads.bam
echo BQSR is applied for ${i}
...
gatk --java-options "-Xmx4G" SelectVariants \
        -R Reference.fna \
        -V ${i}.raw_variants_recal.vcf \
        --select-type  SNP \
        -O ${i}.raw_snps_recal.vcf
echo SNPs is extracted for ${i}

gatk --java-options "-Xmx4G" SelectVariants \
        -R Reference.fna \
        -V ${i}.raw_variants_recal.vcf \
        --select-type INDEL \
        -O ${i}.raw_indels_recal.vcf
echo INDELs is extracted for ${i}
...
-R Reference.fna \
        -V ${i}.raw_snps_recal.vcf \
        -O ${i}.filtered_snps_final.vcf \
        --filter-expression 'QUAL < 30.0 || QD < 2.0 || FS > 60.0 || DP < 10 ' \
        --filter-name lowQualFilter --cluster-window-size 10 --cluster-size 3 --missing-values-evaluate-as-failing
echo SNPs is filtered №2 for ${i}

#Filter Indels
gatk --java-options "-Xmx4G" VariantFiltration \
-R Reference.fna \
        -V ${i}.raw_indels_recal.vcf \
        -O ${i}.filtered_indels_final.vcf \
        --filter-expression 'QUAL < 30.0 || QD < 2.0 || FS > 60.0 || DP < 10 ' \
        --filter-name lowQualFilter --cluster-window-size 10 --cluster-size 3 --missing-values-evaluate-as-failing
echo INDELs is filtered №2 for ${i}



# пересекаем с референсом
bedtools intersect -wb -a ${i}.filtered_snps_final.vcf  \
-b ~/Genomes_mol_typing/Chlamydiales/reference/GCF_000008725.1_ASM872v1_genomic.gff  \
-header >  ${i}.named.snp.vcf
bedtools intersect -wb -a ${i}.filtered_indels_final.vcf \
-b ~/Genomes_mol_typing/Chlamydiales/reference/GCF_000008725.1_ASM872v1_genomic.gff  \
...
#mv ${i}.snp.Chl.p_multianno.vcf ~/Genomes_mol_typing/Chlamydiales/calling/Italy
#mv ${i}.indel.Chl.p_multianno.vcf ~/Genomes_mol_typing/Chlamydiales/calling/Italy
